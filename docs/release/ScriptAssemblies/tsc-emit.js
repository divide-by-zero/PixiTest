...
(function (game) {
    ...
    var InputMovementSystem = /** @class */ (function (_super) {
        ...
        InputMovementSystem.prototype.OnUpdate = function () {
            ...
            this.world.forEach([game.MoveSpeed, game.MoveWithInput, game.Boundaries, ut.Core2D.TransformLocalPosition], function (speed, input, bound, position) {
                ...
                localPosition.x = Math.max(bound.minX, Math.min(bound.maxX, localPosition.x));
                localPosition.y = Math.max(bound.minY, Math.min(bound.maxY, localPosition.y));
